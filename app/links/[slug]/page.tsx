...
export default async function LinkPage({ params }: Props) {
  ...
  if (!post || !post.published || post.type !== "link") notFound()
    ...
    <article>
      <header className="post-header">
        <h1 className="post-title">
          ...
          {post.link && (
            <Link href={`/links/${post.pk}`} className="permalink-glyph permalink-glyph-title" title="Permalink">★</Link>
          )}
        </h1>
      ...
      <div className="post-footer">
        <Link href="/links" className="back-link-text">← Links</Link>
        {session && (
